...
type Page = { data: Zone[] };
@Component({ selector: 'app-super-admin-zones', standalone: true, imports: [CommonModule, TranslatePipe, TableComponent, FormDialogComponent, SelectControlComponent], templateUrl: './zones.html', changeDetection: ChangeDetectionStrategy.OnPush })
export class SuperAdminZonesComponent implements OnInit {
  ...
  readonly cities = signal<City[]>([]); readonly cityId = signal(''); readonly rows = signal<Zone[]>([]); readonly loading = signal(false); readonly dialog = signal(false); readonly editing = signal<Zone | null>(null); readonly saving = signal(false);
  ...
  private async load() { if (!this.cityId()) return; this.loading.set(true); try { const r = await this.api.get<Page>('/api/v1/dashboard/zones', { params: { cityId: this.cityId(), page: 1, limit: 100 } }); this.rows.set(r.data.data); } catch (e) { this.notify.error(apiErrorMessage(e, this.lang.t('common.unexpectedError'))); } finally { this.loading.set(false); } }
}
