...
export class SuperAdminSubcategoriesComponent {
  ...
  async save(value: Record<string, unknown>) { const nameAr = String(value['nameAr'] ?? '').trim(), nameEn = String(value['nameEn'] ?? '').trim(); if (!this.mainId() || !nameAr || !nameEn) return; this.saving.set(true); const body = { mainCategoryId: this.mainId(), translations: [{ locale: 'ar', name: nameAr }, { locale: 'en', name: nameEn }], status: value['status'], displayOrder: Number(value['displayOrder']) }; try { const row = this.editing(); if (row) await this.api.patch(`/api/v1/dashboard/subcategories/${row.id}`, body, { params: { cityId: this.cityId() } }); else await this.api.post('/api/v1/dashboard/subcategories', body, { params: { cityId: this.cityId() } }); this.open.set(false); await this.loadRows(); this.notify.success(this.language.t('common.success')); } catch (err) { this.notify.error(apiErrorMessage(err, this.language.t('common.unexpectedError'))); } finally { this.saving.set(false); } }
