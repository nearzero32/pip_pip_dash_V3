...
export class StoreCommissionsComponent implements OnInit, OnDestroy {
  ...
  fields(): FormField[] { return [{ name: 'platformCommissionRate', label: 'نسبة عمولة المنصة (%)', type: 'number', required: true, width: 'full', hint: 'أدخل رقمًا صحيحًا بين 0 و100.' }, { name: 'reason', label: 'سبب التغيير', type: 'textarea', required: true, width: 'full', placeholder: 'مثال: تعديل اتفاق العمولة مع المتجر' }, { name: 'note', label: 'ملاحظة', type: 'textarea', width: 'full', placeholder: 'تفاصيل إضافية اختيارية' }]; }
